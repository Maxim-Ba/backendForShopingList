    ...
    ws.on("message", async (m) => {
      ...
      switch (mesData.event) {
        ...
        case "updateUser":
          ...
          const usersList = await WSServise.updateUser(
            mesData.listID,
            mesData.userEmail
          );
          ...
          wss.clients.forEach((client) => {
            if (
              client.listID === mesData.listID ||
              client.userID === users[users.length - 1]
            ) {
              client.send(
                JSON.stringify({
                  event: "updateUser",
            ...
            if (client.userID === users[users.length - 1]) {
              async () => {
                const newListThatWasShared =
                  await WSServise.getNewListThatWasShared(
                    ws.userID,
                    client.userID
                  );
                client.send(
        ...
        case "updateGroups":
          ...
          const newGroups = await WSServise.updateGroups(
            mesData.listID,
            mesData.groups
          );
          wss.clients.forEach((client) => {
        ...
        case "updateDeletedAndGroups":
          const { groups, deleted } = await WSServise.updateDeletedAndGroups(
            mesData.listID,
            mesData.groups,
            mesData.deleted
          );
